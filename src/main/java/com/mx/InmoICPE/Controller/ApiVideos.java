...
import com.mx.InmoICPE.Service.ImpVideos;

@RestController
@RequestMapping(path = "api/videos")
@CrossOrigin("*")
public class ApiVideos {
	@Autowired ImpVideos imp;
	
	//URL: http://localhost:9005/api/videos
	@GetMapping
	...
	@PostMapping
	public ResponseEntity<Videos> create(@RequestBody Videos body){
		body = (Videos) imp.guardar(body);
		if (body != null) {
	...
	public ResponseEntity<Videos> update(@PathVariable Integer id, @RequestBody Videos body){
		Videos found = (Videos) imp.buscar(id);
		if (found != null) {
